...
function popUpWindow(id, left, top, width, height)
{
  ...
  URLStr='../modules/mod_accout_ledger_details.php?id='+id;
...
<tr><td valign="top"  >
	...
	if( $sub_branch=='' ){
		...
		if($row_user_chk['sub_branch_id']==0){
		    ...
		    $sql="select * from tbl_account_trans_details where  frm_branch ='$brid'  and voucher_date between '$std' and '$end' ";
			
		}else{
			
			$sql="select * from tbl_account_trans_details where   frm_sub_branch=".$row_user_chk['sub_branch_id']." OR 
		...
		$sql="select * from tbl_account_trans_details where  frm_sub_branch='$sub_branch' OR to_sub_branch='$sub_branch' AND voucher_date between '$std' and '$end' ";
...
$count=mysql_num_rows($result);
			...
			<td   align="center" class="rFont9">
           	 ...
           	 <a href="../modules/mod_accout_ledger_details.php?id=<?php echo $row_user['acc_tranc_id']; ?>" target="_blank" onclick=	
             "popUpWindow('<?php echo $row_user['acc_tranc_id']; ?>',110,100,700,500);return false">
			<?php echo $row_user['voucher_no']; ?> </a>
            
            </td>
			<td   align="center" bordercolor="#E9E9E9"><span class="hotspot" onmousedown="tooltip.show('<?php echo $row_user['acc_tranc_id']?>');" onmouseout="tooltip.hide();">D:Click here</span></td>
