...
$stkid=$_SESSION['stk_id'];
		 ...
		 $sql="select c.*,s.* from tbl_stock_master s INNER JOIN tbl_closing_stock c ON s.stk_id=c.stk_id WHERE c.stk_id=".$stkid ." AND c.branch_id=".$_GET['brd']." and c.closing_stk>0";
		 if($sub_br!=''){
		 	$sql=$sql." and c.sub_br_id=$sub_br";
		 }
		...
		while($row_sql=mysql_fetch_assoc($result_sql)){
			...
			$flg=0;	
				for($j=0;$j<$i;$j++){
							if($sub_br_id[$j]==$row_sql['sub_br_id']){
...
<tr><td valign="top"  >
        <table border="0" align="center" cellpadding="3" width="88%">
          ...
          <tr bgcolor="#666666">
          <td height="25" align="center" width="197" class="style3_head">Sub Branch</td>
            ...
            ?>
             ...
             <td   align="left" bordercolor="#E9E9E9" class="text1_cheq">
			 <?php
		 
			 $sql="select sub_branch_name from tbl_sub_branches where id=".$sub_br_id[$k];
			 $result=mysql_query( $sql);
			 
			 $row=mysql_fetch_assoc($result);
			 
			 echo $row['sub_branch_name'];
