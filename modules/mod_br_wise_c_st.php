...
$i=0;
$flg=0;
$stkid=$_SESSION['stk_id'];
		
		 $sql="select c.*,s.* from tbl_stock_master s INNER JOIN tbl_closing_stock c ON s.stk_id=c.stk_id WHERE c.stk_id=".$stkid ." AND c.region_id=".$_GET['reg']." and c.closing_stk>0";
		 if($br!=''){
		 	$sql=$sql." and c.branch_id=$br";
		 }
		...
		while($row_sql=mysql_fetch_assoc($result_sql)){
			...
			$flg=0;	
				for($j=0;$j<$i;$j++){
							if($branch_id[$j]==$row_sql['branch_id']){
...
<tr><td valign="top"  >
        <table border="0" align="center" cellpadding="3" width="88%">
          ...
          <tr bgcolor="#666666">
          <td height="25" align="center" width="197" class="style3_head">Branch</td>
            ...
            ?>
             ...
             <td   align="left" bordercolor="#E9E9E9" class="text1_cheq"><a href="mod_sub_br_wise_c_st.php?brd=<?php echo $branch_id[$k] ?>">
             
             <?php
		 
			 $sql="select branch_name from tbl_branch_master where branch_id=".$branch_id[$k];
